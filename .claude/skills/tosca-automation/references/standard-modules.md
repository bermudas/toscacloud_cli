...
- **The JS string must be bracket-free at the top level.** TBox's dynamic-value parser interprets `{` and `[` at the value root as expression delimiters. Array-spread at the top level (`[...document.querySelectorAll(...)].length`) produces `Value never closed before opening new one: {SCRIPT[...` parse errors even though no `{SCRIPT[...]}` expression is actually present. Use `document.querySelectorAll(...).length` (no spread), or wrap the code in an IIFE (`(function(){ ... })()`) so brackets live inside a function body.
...
- **`moduleReference.metadata.engine = "Framework"`**, not `"Html"`. The Framework engine is what dispatches `SpecialExecutionTask`s (ExecuteJavaScript, VerifyJavaScriptResult, OpenUrl, CloseBrowser). Getting this wrong can emit engine warnings or subtle misbehavior.
- **Double-quote escaping inside the JS string**: if the JS body itself contains `"`, escape by tripling (`"""http://x"""`) per Tosca Cloud docs. Single quotes in the JS have no escape requirement.
