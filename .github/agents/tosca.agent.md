...
7. **Confirm the write landed** — after every `cases patch`, `cases update`, `modules update`, `blocks update`, `inventory patch`: GET the artifact and assert the `version` field bumped AND the specific edited field actually changed. The CLI's `✓ patched` line and an HTTP 204/`{}` response are not proof — they only confirm the request was accepted. MBT silently drops unsupported JSON Patch ops (deep paths into nested step trees, `remove` on array elements, `move`); Inventory v3 silently ignores MBT-shape bodies. If the confirm-GET shows no change, fall back to full PUT (`cases update --json-file …`).
8. **Validate** — re-run and confirm the previously failing step now passes.
9. **Report** — IDs, folder placement, remaining gaps.
...
| `return` required in Verify JavaScript Result | The JavaScript attribute must include an explicit `return` statement; without it the agent captures `undefined` and the Result mismatches. **ALL `{` and `}` anywhere in the JS string are TBox expression delimiters** — not just at the top level. Even callback bodies like `function(a){return a.text}` fail: TBox reads `{return a.text}` as an unknown command and returns `No suitable value found for command return a.text.` The IIFE pattern `(function(){...})()` is equally unsafe. **Write all VJS JS brace-free**: use arrow expressions (`a => a.text`), ternary+comma (`el ? (el.click(), 'clicked') : 'not found'`), and `var x = expr;` statements. `[` at the very start of the value is also a delimiter — inside expressions `Array.from(...)` is fine. |
...
| `el.click()` in VJS JS can be silently intercepted | CMS frameworks (Drupal `data-extlink=""`, React/Vue synthetic events) attach click listeners that call `event.preventDefault()`, cancelling navigation. The VJS JS returns its value normally (no JS error), but `window.location.href` is unchanged. **Diagnosis**: VJS step reports "clicked", subsequent URL-verify step fails with the original URL. **Fix**: `window.location.href = el.href` — direct property assignment bypasses all event handlers. |
| VJS probe for robust conditional CloseBrowser | Instead of an Html-module element Verify (which requires a scanned module and fails hard if the document isn't open), use a VJS step as the `ControlFlowItemV2 If` condition: `UseActiveTab=False + Title=*<AppName>* + JS: return 'present' + Result Verify 'present'`. When no matching tab exists, VJS silently returns `""` → Verify mismatches → condition false → CloseBrowser skipped. When the tab IS open, JS returns `"present"` → Verify passes → CloseBrowser runs. Works without any scanned module; handles the "browser not running at all" case cleanly. |
...
| MBT PATCH does not support deep JSON pointer paths | `cases patch` with a path like `/testCaseItems/1/items/2/testStepValues/0/value` silently succeeds (204) but makes no change. Same for `remove` on array elements (`/testCaseItems/1/items/3`) and `move`. For step-level edits, use a full `cases update <id> --json-file` (PUT) instead — fetch the current JSON, mutate in Python, remove `version`, then PUT. |
| Always confirm mutations landed | `✓ patched` from the CLI, HTTP 204 from MBT, `{}` from `modules update` — none of these prove the diff was applied. Always follow a write with a GET and assert `version` bumped and the edited field actually changed. Inventory v3 PATCH (PascalCase, wrapper object) silently ignores MBT-shape bodies in the same way. One throwaway probe — `{"op":"replace","path":"/description","value":"probe-N"}` round-trip — is enough to confirm the endpoint is accepting your shape before you batch real edits. |
